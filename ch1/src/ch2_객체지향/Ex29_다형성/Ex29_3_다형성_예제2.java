...
/*
    커피 구매하는 프로그램을 구현한다치고
    그냥 코드를 짜게되면 각 메뉴마다 가격 이름 같은 정보들이 다 다르기 때문에 각 메뉴마다 구매하는 메서드를 일일이 구현해줘야 한다.
    그런데 다형성을 활용하면 개별적인 커피의 구매 메서드를 따로 구현하지 않아도 된다.(그냥 메서드 하나로 모든 메뉴를 구매할수 있게 할 수 있다.)

    방법: 상위 클래스인 coffee 자료형을 매개변수로 전달받으면, 하위 클래스 타입의 참조 변수는 매개변수로 전달될수있다

...
/*
    = ArrayList =
        ...
        위 두가지 방법이 있는데 ArrayList에마 있는 메서드를 사용할세 아니라면 list를 사용하는게 더 좋다
        이유는 ArrayList 처럼 list 인터페이스로 구현한 클래스가 더 있는데 (LinkList)
        2번처럼 자료형을 ArrayList로 해버리면 변수를 재활할수가 없다
    ...
    = 사용가능한 메서드 =
        ...
        **2) add (데이터) : ArrayList 의 마지막 방으로 추가
        3) add (방번호 데이터) : 특정 방에 데이터 추가 (가존것들은 뒤로 한칸씩 밀림)
        **4) get(방번호) : 해당 방번호꺼 데이터 읽기
        *5) remove(방번호) : 해당 방번호 데이터 삭제
        6) addAll(데이터) : 모든 객체를 저장
        7) addAll(방번호, 데이터) : 지정된 방번호 부터 저장 (이걸 돌리려면 '방번호' 만큼 기존 ArrayList에 자료가 들어있어야함)
        //잘 사용하지 않고 다른 방번호 넣을때에는 위에서 if 문을 넣어 한번 걸러줘야한다

        *8) set(방번호, 데이터) : 해당 방에 데이터 변경
        //깊은복사 객체의 내용까지 전부 새롭 복사 => 원본과 복사본이 완전히 독립적
...
public class Ex39_2_ArrayList클래스 {
    public static void main(String[] args) {
        ...
        System.out.println("--------------");
        // 2) List 를 이용한 정렬
        newFruit2.set(0, "banana");
        System.out.println("1: " + newFruit2);

        newFruit2.sort(Comparator.naturalOrder()); // desc 정렬
        System.out.println("2: " + newFruit2);

        newFruit2.sort(String.CASE_INSENSITIVE_ORDER); // 대소문자 구분 없이 asc 정렬
        System.out.println("3: " + newFruit2);

        newFruit2.sort(Comparator.reverseOrder()); //  desc 정렬
        System.out.println("4: " + newFruit2);

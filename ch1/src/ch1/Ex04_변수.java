...
/*
    ...
    - 변수 종류
        ...
        3) 지역 변수(Local variable, lv)
            ...
            멤버변수와 동일한 이름을 가질수 있고, 지역변수가 우선순위

        4) 매개 변수(Parameter)
            메소드에 인자로 전달되는 값을 받는 변수
            지역변수 처럼 사용됨
    ...
    - 변수 생성
        ...
        2) 변수값 초기화 - 변수를 선언할때 빈 박스로 두지 말고, 기본으로 사용할 초기값을 설정해놓자
                        ex) int num = 0;
    ...
    - 변수명 생성 규칙
        ...
        5) 공백 불가
            ex) int bitstudy; (o)
                int bit study; (X) => int bit_study;, int bitStudy; 
...
//        System.out.println(A.num1);
        ...
        A tmpA = new A();
        System.out.println(tmpA.num1); // num1, num2 가 멤버변수와 지역변수로도 다 있지만
...
class A {
    ...
    void aMain() {
        int num1 = 200; // 지역변수
        int num2 = 400; // 지역변수
        System.out.println("num1: " + num1); // 같은 이름의 변수가 '클래스 영역' 에도 있고 '지역'에도 있을때는 제일 가까운쪽꺼 출력함.
